...
def normalize(someList):
    amin = np.min(someList)
    amax = np.max(someList)
    mean = np.mean(someList)
    for i in range(0, len(someList)):
        someList[i] = someList[i] / amax
    return someList


def removereadings(someList):
    return someList[:150]


def customeProcess(someList):
    return removereadings(normalize(someList))
...
xprocessed = np.zeros([numberOfItems, 150, 150], dtype=float)

for i in range(0, numberOfItems):
    for j in range(0, numberOfElectrodes):
        # xprocessed[i][j] = spectrum(xdata[i][j])
        xprocessed[i][j] = customeProcess(spectrum(xdata[i][j]))

xshape = xprocessed.shape
numberOfItems = xshape[0]
numberOfElectrodes = xshape[1]
numberOfReadings = xshape[2]

# plt.plot((xprocessed[6][2]), color='green', linewidth=1)
# plt.show()
# exit(0)
...
X_train = X_train.reshape(X_train.shape[0], numberOfReadings, numberOfReadings, 1)
X_test = X_test.reshape(X_test.shape[0], numberOfReadings, numberOfReadings, 1)
...
# model = Sequential()
# model.add(Convolution2D(32, 3, 3, activation='relu', input_shape=(numberOfReadings,numberOfReadings,1)))
# model.add(Convolution2D(40, 1, activation='relu'))
# model.add(Convolution2D(20, 1, activation='relu'))
# model.add(Convolution2D(10, 1, activation='relu'))
# model.add(Convolution2D(2, (numberOfReadings-2)))
# model.add(Flatten())
...
y_pred = model.predict(X_test.reshape(X_test.shape[0], numberOfReadings, numberOfReadings, 1))
